...
def create_game_schedule(n_players, n_rounds=5, max_games_per_round=10, 
                        max_pairwise_meetings=2, time_limit_seconds=300):
    """
    ...
    Args:
        ...
        time_limit_seconds: Maximum solving time in seconds (default 300 = 5 minutes)
    ...
    # 7. HARD CONSTRAINT: No pair meets more than max_pairwise_meetings times
    for p1, p2 in combinations(players, 2):
        model += meet_count[p1, p2] <= max_pairwise_meetings, f"max_meet_constraint_{p1}_{p2}"
    
    # 8. Bound max_meet (for reporting purposes)
    for p1, p2 in combinations(players, 2):
    ...
    # Priority 1: Minimize max 2-player games per player
    # Priority 2: Minimize variance in pairwise meetings (spread encounters evenly)
    # We can use sum of meet_count to minimize total repeat encounters
    ...
    if violations:
        ...
        print("\nThis suggests the solver didn't find a feasible solution within the time limit.")
        print("Try: (1) Increasing time_limit_seconds, or (2) Relaxing max_pairwise_meetings\n")
...
def test_corner_cases():
    ...
    for n in corner_cases:
        ...
        schedule, stats = create_game_schedule(n, time_limit_seconds=600)
...
if __name__ == "__main__":
    # Test a specific case with longer time limit
    n = 11
    schedule, stats = create_game_schedule(n, max_pairwise_meetings=2, time_limit_seconds=3600)
    if schedule:
