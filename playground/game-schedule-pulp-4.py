...
def create_game_schedule(n_players, n_rounds=5, max_games_per_round=10):
    ...
    # NEW: Variables to count pairs meeting 0, 1, 2, 3+ times
    # This helps us maximize variety in pairings
    meet_at_least = {}
    for p1, p2 in combinations(players, 2):
        for threshold in range(1, n_rounds + 1):
            meet_at_least[p1, p2, threshold] = LpVariable(
                f"meet_at_least_{p1}_{p2}_{threshold}", cat=LpBinary
            )
    ...
    # 10. NEW: Link meet_at_least indicators to actual meeting counts
    # meet_at_least[p1,p2,k] = 1 iff meet_count[p1,p2] >= k
    for p1, p2 in combinations(players, 2):
        for threshold in range(1, n_rounds + 1):
            # If meet_count >= threshold, then meet_at_least = 1
            # This is: meet_count >= threshold * meet_at_least
            # Rearranged: meet_count >= threshold ==> meet_at_least = 1
            # We use: meet_at_least <= meet_count / threshold (but need to bound it)
            # Better formulation: meet_count - threshold + 1 <= M * meet_at_least
            # where M is large enough (e.g., n_rounds)
            M = n_rounds
            model += meet_count[p1, p2] >= threshold * meet_at_least[p1, p2, threshold], \
                f"meet_threshold_lower_{p1}_{p2}_{threshold}"
            model += meet_count[p1, p2] <= threshold - 1 + M * meet_at_least[p1, p2, threshold], \
                f"meet_threshold_upper_{p1}_{p2}_{threshold}"
    
    # Objective: Lexicographic priorities
    # Priority 1 (weight 1,000,000): Minimize max 2-player games per player
    # Priority 2 (weight 1,000): Minimize max pairwise meetings
    # Priority 3 (weight 1): Minimize high-frequency meetings (spread encounters)
    
    # To spread encounters, we penalize pairs meeting 3+ times heavily
    penalty_for_high_meetings = lpSum(
        meet_at_least[p1, p2, 3] * 10 +  # Heavy penalty for 3+ meetings
        meet_at_least[p1, p2, 2] * 1     # Light penalty for 2+ meetings
        for p1, p2 in combinations(players, 2)
    )
    
    model += (1000000 * max_two_player + 
              1000 * max_meet + 
              penalty_for_high_meetings), "objective"
