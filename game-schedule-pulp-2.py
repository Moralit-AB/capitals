...
def analyze_feasibility(n_players, n_rounds=5):
    ...
    if remainder == 1:
        # n = 3k+1: need (k-1) 3P games + 2 2P games
        # Example: 10 = 3*3+1 → 2 games of 3P + 2 games of 2P
        three_player_games = (n_players - 4) // 3
        two_player_games = 2
    else:  # remainder == 2
        # n = 3k+2: need k 3P games + 1 2P game
        # Example: 11 = 3*3+2 → 3 games of 3P + 1 game of 2P
...
def create_game_schedule(n_players, n_rounds=5, max_games_per_round=10):
    ...
    # 9. FIXED: Constrain exact number of 2-player AND 3-player games per round
    for r in rounds:
        ...
        # Number of 3-player games (games used but not 2-player)
