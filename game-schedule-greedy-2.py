...
def create_game_schedule_greedy(n_players, n_rounds=5, max_pairwise_meetings=2, 
    ...
    for attempt in range(max_attempts):
        ...
        if schedule is not None:
            ...
            if score < best_score:
                ...
                if violations == 0 and max_2p == feasibility['min_max_2p_per_player']:
                    print(f"✓ Found optimal schedule (attempt {attempt + 1}/{max_attempts})")
                    break
        
        if (attempt + 1) % 10 == 0:
            print(f"  Attempt {attempt + 1}/{max_attempts}... (best max_2p: {best_stats['max_two_player_games'] if best_stats else 'N/A'})")
    ...
    theoretical_2p = best_stats['feasibility']['min_max_2p_per_player']
...
def _generate_round_2p_fair(players, feasibility, meeting_count, two_player_count, 
    ...
    Strategy:
    1. First select 2P games from players with lowest 2P count
    2. Then fill remaining with 3P games minimizing pairwise meetings
    """
    ...
    # STEP 1: Generate 2-player games FIRST, selecting players with lowest 2P count
    for _ in range(n_2p):
        if len(available) < 2:
            return None
    ...
    # STEP 2: Generate 3-player games from remaining players
    for _ in range(n_3p):
        if len(available) < 3:
            return None
...
def _select_2p_game_fair(available, meeting_count, two_player_count, max_pairwise_meetings):
    ...
    Priority:
    1. Minimize max 2P count of the two selected players
    2. Among ties, don't violate pairwise constraint
    3. Among ties, minimize existing meetings
    """
    
    if len(available) < 2:
        return None
    
    # Group players by their current 2P count
    by_2p_count = defaultdict(list)
    for p in available:
        by_2p_count[two_player_count[p]].append(p)
    
    # Try players with lowest 2P count first
    sorted_counts = sorted(by_2p_count.keys())
    
    best_game = None
    best_score = float('inf')
    
    # Strategy: try combinations starting with lowest 2P counts
    for max_count_allowed in sorted_counts:
        # Get all players with 2P count <= max_count_allowed
        candidate_players = []
        ...
        # Try all pairs from these candidates
        for p1, p2 in combinations(candidate_players, 2):
            pair = (min(p1, p2), max(p1, p2))
            
            # Check if this pair would violate constraint
            current_meetings = meeting_count[pair]
            if current_meetings >= max_pairwise_meetings:
                continue
            
            # Score: prioritize lower max 2P count, then fewer existing meetings
        ...
        # If we found a valid game at this level, return it
        if best_game is not None:
            return best_game
    
    # If no valid game found without violations, try to find any valid game
    # This is a fallback that allows slightly worse 2P balance
...
def _select_3p_game(available, meeting_count, max_pairwise_meetings):
    ...
    Priority:
    1. Don't violate pairwise constraint
    2. Minimize maximum pairwise meetings in this game
    3. Minimize total pairwise meetings in this game
    """
...
def print_schedule(schedule, stats, show_all_pairs=False):
    ...
    # Show individual player 2P counts for verification
    print("\n  Individual player 2P counts:")
    for p in sorted(stats['two_player_counts'].keys()):
        count = stats['two_player_counts'][p]
        print(f"    Player {p}: {count} times")
    print()
...
def test_corner_cases():
    ...
    for n in corner_cases:
        ...
        schedule, stats = create_game_schedule_greedy(n, max_attempts=100)
        ...
        if schedule:
            ...
            if achieved == theoretical:
                ...
                print(f"✗ Gap: theoretical optimum={theoretical}, achieved={achieved}")
